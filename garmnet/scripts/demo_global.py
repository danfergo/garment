...
with K.get_session():
    ...
    m.build(True)
    m.compile()
    m.load_weights(m.garment_detector, 'global_training')
    ...
    while 1:
        ...
        np_img = imread('/home/danfergo/Transferências/cloC00024.png')
        np_img = resize(np_img, (224, 224, 3))
        # print(np_img.shape)
        np_img = data[0][0]

        np_img = rotate(np_img, 2)
        ...
        pred = m.predict_global(np.array([np_img]))
        # plot.show_results(np.array([np_img]), pred)
        if pred[0][0] is not None:
            pygame.display.set_caption(dataset.categories[pred[0][0]])
        for p in pred[0][3]:
